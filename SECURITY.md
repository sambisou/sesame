...
## The Keychain assistant: only `sesame-keychain` reads silently

Since 0.5.0, Sésame ships a small signed helper, `sesame-keychain` (source: `macos/Sources/SesameKeychain`, built into `Sésame.app/Contents/MacOS/sesame-keychain`). Its only job is to *read* a Keychain item — `get` (the secret) or `has` (just its presence) — it has no command that writes, deletes, or changes anything. When this assistant is present and signed, `sesame add` (and the Sésame window) create each new item with `-T <path to that exact binary>`: the Keychain then trusts **only that one file**, verified by macOS through its code signature and content hash, not by its path — a different program dropped at the same path, or a modified copy, would not match and would not be trusted. Reads by the assistant are then silent, with no dialog. `security`, and everything else on your Mac, is never made trusted by this: reading the same item any other way still triggers the system Keychain dialog, exactly as before 0.5.0.
...
Items registered before 0.5.0 (or before 0.3.0, when items carried no trusted application at all) keep their old ACL: `sesame doctor` flags them, and the first read after upgrading still shows the Keychain dialog — once, not at every login. At that one prompt, click **Always Allow**: this is now the correct answer, because the requester macOS names in the dialog is `sesame-keychain`, not `security`. Re-registering the site (`sesame add <site>` or the Sésame window) recreates the item with `-T` from the start and skips even that first click.
