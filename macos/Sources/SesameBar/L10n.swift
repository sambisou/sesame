...
enum L10n {
    ...
    private static let table: [String: (fr: String, en: String)] = [
        ...
        // MARK: demandes d'accès (fenêtre flottante + rangée du panneau)
        "panel_asks": ("À vous de répondre", "Waiting for you"),
        "win_ask_title": ("Sésame — demande d'accès", "Sésame — access request"),
        "win_ask_domain_title": ("Sésame — nouveau domaine", "Sésame — new domain"),
        "ask_access_title": ("Claude demande « %@ »", "Claude is asking for “%@”"),
        "ask_domain_title": ("Nouveau domaine pour « %@ »", "New domain for “%@”"),
        "ask_access_line": ("%@ veut se connecter à %@ avec vos identifiants. Sésame les tape lui-même ; Claude ne les voit pas.",
                            "%@ wants to sign in to %@ with your credentials. Sésame types them itself; Claude never sees them."),
        "ask_reason_line": ("Motif : %@", "Reason: %@"),
        "ask_channel_line": ("Dans %@.", "In %@."),
        "ask_always": ("Ne plus me demander pour « %@ » (règle « Automatique »)", "Don't ask me again for “%@” (rule “Automatic”)"),
        "ask_allow": ("Autoriser", "Allow"),
        "ask_deny": ("Refuser", "Deny"),
        "ask_footer": ("Cette fenêtre reste au-dessus des autres, sur tous les bureaux, et dans le menu Sésame jusqu'à votre réponse.",
                       "This window stays above the others, on every desktop, and in the Sésame menu until you answer."),
        "ask_row_detail": ("%@ · %@", "%@ · %@"),
        "ask_show_help": ("Ramener la fenêtre au premier plan", "Bring the window to the front"),
